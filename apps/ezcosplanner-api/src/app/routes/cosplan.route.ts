...
import { updateCosplanSchema, updateCosplanStatusSchema } from '../schemas/cosplan';
import { JsonSchemaToTsProvider } from '@fastify/type-provider-json-schema-to-ts';
import { deleteCosplan, updateCosplan, updateCosplanStatus } from '../services/cosplan.service';
...
export default async function (fastify: FastifyInstance) {
  ...
  fastify
    ...
    .patch(
      ...
      async (request) =>
        await updateCosplanStatus(fastify.prisma, request.params.id, request.body)
    );

    fastify.delete(
      '/cosplan/:id',
      async (request: FastifyRequest<{ Params: { id: number } }>) => {
        return await deleteCosplan(fastify.prisma, +request.params.id);
      }
    );
}
